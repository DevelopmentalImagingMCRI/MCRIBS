...
def writeSurf(surfStruct: dict, fileName: str, geometryIMGFileName=None, convertToTKR=False):
    ...
    if geometryIMGFileName is None:
        ...
        volume_info['cras'] = numpy.array(numpy.matrix(numpy.vstack((volume_info['xras'], volume_info['yras'], volume_info['zras']))) * numpy.matrix((numpy.array(geometryIMG.shape[0:3]) / 2 - RAS2VOX[0:3, 3]) * geometryIMG.header.get_zooms()[0:3]).T).ravel()
        if convertToTKR:
