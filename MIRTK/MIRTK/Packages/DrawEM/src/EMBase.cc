...
namespace mirtk {
    ...
    void EMBase::InitialiseGMMParameters(int n, double *m, double *s,
					 double *c) {
	int i;
	...
	for (k = 0; k < _number_of_tissues; k++) {
		...
		std::
		    cerr << "Division by zero while computing tissue mean!"
		    << std::endl;
		exit(1);
	    }
	}
	...
	for (k = 0; k < _number_of_tissues; k++) {
		...
		if (_mask.Get(it->first) == 1) {
		    sigma_num[k] +=
			it->second * pow(_input.Get(it->first) - _mi[k],
					 2);
		}
	    }
	}
	...
	if (_postpen)
	    pptr = _postpenalty.GetPointerToVoxels();
	...
	for (i = 0; i < _number_of_voxels; i++) {
		...
		if (denominator == 0) {
		    ...
		    std::
			cerr <<
			"Division by 0 while computing probabilities at voxel "
			<< x << "," << y << "," << z << std::endl;
		}
	    } else {
		for (k = 0; k < _number_of_tissues; k++) {
	...
	for (k = 0; k < _number_of_tissues; k++) {
		...
		std::
		    cerr << "Tissue " << k <<
		    ": Division by zero while computing tissue mean!" <<
		    std::endl;
		exit(1);
	    }
	    if (uniform_prior)
		_c[k] = 1.0 / _number_of_tissues;
	    else
    ...
    double EMBase::IterateGMM(int iteration, bool equal_var,
			      bool uniform_prior) {
	if (iteration > 1)
	    this->EStepGMM();
	if (equal_var)
	    this->MStepVarGMM(uniform_prior);
	else
	    this->MStepGMM(uniform_prior);
	PrintGMM();
	...
	if (i < _number_of_tissues) {
	    ...
	    std::
		cerr <<
		"HashProbabilisticAtlas::Write: No such probability map" <<
		std::endl;
	    exit(1);
	}
    }
	...
	std::
	    cout << "Writing GaussianDistributionParameters: " << file_name
	    << std::endl;
	...
	if (flag) {
		...
		for (l = 0; l < 1 /*_input.GetNumberOfChannels()*/ ; l++) {
		    fileOut << _mi[k];	//.Get(l);
		    if (l == 0 /*_input.GetNumberOfChannels() - 1*/ )
			fileOut << ")" << std::endl;
		    else
			fileOut << ", ";
		}
		...
		for (l = 0; l < 1 /*_input.GetNumberOfChannels()*/ ; l++) {
		    //fileOut << "(";
		    for (m = 0; m < 1 /*_input.GetNumberOfChannels()*/ ;
			 m++) {
	...
	if ((temp > 1) || (temp < 0)) {
	    std::
		cerr <<
		"Could not compute likelihood, probability out of range = "
		<< temp << std::endl;
	    exit(1);
	}
	return -log(temp);
