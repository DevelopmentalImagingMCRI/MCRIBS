...
if [ ! -f $sdir/MADs/$subj-subspace.nii.gz ];then
	...
	for atnum in {01..10};do
		...
		if [ ! -f $sdir/transformations/T2-$subj-$atlas.nii.gz ];then 
			ms=$sdir/template/$str/$subj.nii.gz
			run mirtk transform-image $DRAWEMDIR/atlases/MCRIBs/T2/$atlas.nii.gz $sdir/transformations/T2-$subj-$atlas.nii.gz -target N4/$subj.nii.gz -dofin dofs/$subj-$atlas-n.dof.gz -interp BSpline 
		fi
