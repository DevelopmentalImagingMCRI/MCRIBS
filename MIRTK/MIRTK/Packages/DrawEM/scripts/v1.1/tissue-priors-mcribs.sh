...
[ $# -ge 2 ] || { echo "usage: $(basename "$0") <subject> <atlas_name> [<#jobs>]" 1>&2; exit 1; }
subj=$1
atlasname=$2
njobs=1
...
#registration
dof=dofs/$subj-template-n.dof.gz
mkdir -p $(dirname "${dof}")

if [ ! -f $dof ];then
  run mirtk register N4/$subj.nii.gz $DRAWEMDIR/atlases/$atlasname/T2/template.nii.gz -dofout $dof -parin $DRAWEMDIR/parameters/ireg.cfg -threads $njobs
fi
...
if [ $needtseg -eq 1 -a ! -f $sdir/tissue-initial-segmentations/$subj.nii.gz ];then
    ...
    run mirtk transform-image $DRAWEMDIR/atlases/$atlasname/atlas-9/prob$strnum.nii.gz $strems -dofin $dof -target N4/$subj.nii.gz -interp Linear
